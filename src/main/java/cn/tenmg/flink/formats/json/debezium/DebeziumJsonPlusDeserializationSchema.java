...
import cn.tenmg.flink.formats.json.debezium.DebeziumJsonPlusDecodingFormat.ReadableMetadata;
 ...
 */
@Internal
public final class DebeziumJsonPlusDeserializationSchema implements DeserializationSchema<RowData> {
	...
	/**
	 * Flag indicating whether to convert a delete record to a update record
	 * (default: true)
	 */
	private final boolean convertDeleteToUpdate;

	public DebeziumJsonPlusDeserializationSchema(DataType physicalDataType, List<ReadableMetadata> requestedMetadata,
			TypeInformation<RowData> producedTypeInfo, boolean schemaInclude, boolean ignoreParseErrors,
		...
		this.jsonDeserializer = new JsonRowDataDeserializationSchema(jsonRowType,
				// the result type is never used, so it's fine to pass in the produced type
				// info
				producedTypeInfo, false, // ignoreParseErrors already contains the functionality of
				// failOnMissingField
	...
	public void deserialize(byte[] message, Collector<RowData> out) throws IOException {
		...
		try {
			...
			if (OP_CREATE.equals(op) || OP_READ.equals(op)) {
				...
				after.setField(after.getArity() - 1, payload.getField(2));
				...
				if (convertDeleteToUpdate) {
					...
					after = new GenericRowData(RowKind.UPDATE_AFTER, arity);
					for (int i = 0; i < arity; i++) {
						after.setField(i, before.getField(i));
					}
					after.setField(arity - 1, payload.getField(2));
					emitRow(row, before, out);
					emitRow(row, after, out);
				} else {
					before.setField(before.getArity() - 1, payload.getField(2));
					before.setRowKind(RowKind.DELETE);
					emitRow(row, before, out);
				}
			} else {
				if (!ignoreParseErrors) {
					throw new IOException(format("Unknown \"op\" value \"%s\". The Debezium JSON message is '%s'", op,
							new String(message)));
				}
			}
		} catch (Throwable t) {
	...
	public boolean equals(Object o) {
		...
		DebeziumJsonPlusDeserializationSchema that = (DebeziumJsonPlusDeserializationSchema) o;
		return Objects.equals(jsonDeserializer, that.jsonDeserializer) && hasMetadata == that.hasMetadata
				&& Objects.equals(producedTypeInfo, that.producedTypeInfo) && schemaInclude == that.schemaInclude
	...
	private static RowType createJsonRowType(DataType physicalDataType, List<ReadableMetadata> readableMetadata,
		...
		DataType payload = DataTypes.ROW(DataTypes.FIELD("before", physicalDataType),
				DataTypes.FIELD("after", physicalDataType), DataTypes.FIELD("op", DataTypes.STRING()));

		// append fields that are required for reading metadata in the payload
		final List<DataTypes.Field> payloadMetadataFields = readableMetadata.stream().filter(m -> m.isJsonPayload)
				.map(m -> m.requiredJsonField).distinct().collect(Collectors.toList());
		...
		final List<DataTypes.Field> rootMetadataFields = readableMetadata.stream().filter(m -> !m.isJsonPayload)
				.map(m -> m.requiredJsonField).distinct().collect(Collectors.toList());
	...
	private static MetadataConverter[] createMetadataConverters(RowType jsonRowType,
			List<ReadableMetadata> requestedMetadata, boolean schemaInclude) {
		return requestedMetadata.stream().map(m -> {
			if (m.isJsonPayload) {
	...
	private static MetadataConverter convertInPayload(RowType jsonRowType, ReadableMetadata metadata,
			boolean schemaInclude) {
		if (schemaInclude) {
